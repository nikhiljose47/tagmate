...
@Component({
  ...
  styles: [`
    .empty-state {
      ...
      background:
        linear-gradient(135deg, color-mix(in srgb, var(--tm-surface, #fff) 92%, transparent), color-mix(in srgb, var(--tm-primary, #6366f1) 6%, var(--tm-surface, #fff))),
        var(--tm-surface, #fff);
    ...
    .empty-title { margin: 0; font-size: 17px; font-weight: 800; color: var(--tm-text, #334155); }
    .empty-sub { max-width: 34ch; margin: 0; font-size: 14px; color: var(--tm-muted, #94a3b8); line-height: 1.45; }
    .empty-action {
      min-height: 38px;
      margin-top: 8px;
      border: 0;
    ...
    @keyframes empty-pulse {
      0%, 100% { transform: scale(1); box-shadow: 0 0 0 0 color-mix(in srgb, var(--tm-primary, #6366f1) 18%, transparent); }
